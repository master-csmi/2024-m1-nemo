...
In this part eight simulation is made, one squirmer's parameters are fixed for every simulation 
excluding $\beta$ which vary, the other one have his
initial orientation and $\beta$ varied.\\
The three $\beta$ parameters used are:
\begin{equation*}
   \beta_1 = 0, \quad \beta_2 = 7.5 \quad \beta_3 = -7.5
\end{equation*}
The left squirmer is the fixed one, his radius is $a=0.15$, his coordinates are $(\frac{-a}{1.1}, 0)$,
 his initial orientation $\theta_1$ is $\frac{\pi}{2}$ and his $\beta$ vary between $\beta_1$, $\beta_2$ and $\beta_3$.\\
The right one is the varying one, his radius is $a$, his coordinates are $(\frac{a}{1.1}, 0)$,
 his $\beta$ vary between $\beta_1$, $\beta_2$ and $\beta_3$ and his initial orientation $\theta_2$ vary between
 $\frac{\pi}{2}$, $\frac{-\pi}{2}$, $\frac{3\pi}{4}$, $\frac{-3\pi}{4}$, $\frac{\pi}{4}$, $\frac{-\pi}{4}$, $\pi$ and $2\pi$.\\
We will then compare our results with those found in previous studies.
\begin{center}
   Behavior observed in previous study.
   \includegraphics[width=1\textwidth]{Presentation/V0/images/stark_behavior.png}\\
   \cite{Stark}\\
   $u_i$ and $u_j$ the current orientational vector and the directional rotation upon interaction in red.  
\end{center}
...
Each squirmer gradually takes his distance from the other,
 it is in line with the study\cite{Stark}.

\subsubsection{$\theta_2 = \frac{-\pi}{2}$}
\begin{center}
   \includegraphics[width=1\textwidth]{graphs/simulations/twosquirmerinter/sq2.-pi.2.png}
\end{center}
Our results are consistent with the previous study when both squirmers have a $\beta \le 0$,
while when $\beta > 0$ our results diverge, one can see the steric forces pushing each squirmer from one another
as attended but their orientation do not align with the study in that case.

\subsubsection{$\theta_2 = \frac{3\pi}{4}$}
\begin{center}
   \includegraphics[width=1\textwidth]{graphs/simulations/twosquirmerinter/sq2.3pi.4.png}
\end{center}
Our results converge from those found in the previous study when $\beta \ge 0$.\\
When $\beta < 0$, one can observe that the right squirmer move closer to the left one while the left one moves away,
at the end of the simulation the right one is behind the left one. This is not consistent with the results of the previous study.

\subsubsection{$\theta_2 = \frac{-3\pi}{4}$}
\begin{center}
   \includegraphics[width=1\textwidth]{graphs/simulations/twosquirmerinter/sq2.-3pi.4.png}
\end{center}
Our simulation align with the previous study when $\beta = 0$.\\
When $\beta < 0$, the right squirmer moves towards the left squirmer and end behind it
 while when $\beta > 0$, the orientation of the left squirmer does not align with the 
 orientational vector observed in the previous study.\\
Our results converge with the pevious study only with $\beta = 0$ when $\theta_2 = \frac{-3\pi}{4}$.
