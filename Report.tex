...
\begin{itemize}
   \item \texttt{export\_data\_csv()} which takes a file name and squirmers data and export it.
   \item \texttt{read\_csv\_file()} which takes a file name and return the data, we don't use it but it can be usefull in the future.
\end{itemize}

\subsection*{interactingsquirmers}
The second file contains the \texttt{Interactingsquirmers} which takes:
\begin{itemize}
...
\begin{itemize}
   ...
   \item \texttt{distance\_sq()} return $(Dx, Dy)$ the direction vector of the $Magnetozoon1$ to the $Magnetozoon2$ 
   and the distance between the two.
   \item \texttt{forcesLubrification()} and \texttt{torquesLubrification()} each takes an argument $choice$ (1 for squirmer1 and 2 for squirmer2)
   ...
   for each time step \texttt{dt} with the forces and torques, then puts all of the usefull informations in a list \texttt{history}
   at each time step \texttt{dt\_out}. The function then returns \texttt{history}.
   \item \texttt{plot\_squirmers\_position(history, filename, dir)} and \texttt{plot\_dist\_sq(history, filename, dir)} each take \texttt{history}, a file name and a directory name
   they then plot the position of the $Magnetozoons$ and the distance between them respectively over time.
   \item \texttt{run()} verify if the $Magnetozoons$ are in the square then execute \texttt{loop\_time()}, export the data into a csv file
   in the \texttt{csv\_files} directory and then plot the positions and distances of the $Magnetozoons$ in the \texttt{graphs}.
\end{itemize}
