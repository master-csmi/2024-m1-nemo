...
\end{equation*}
The left squirmer is the fixed one with a radius of $a=0.15$, placed at $(\frac{-a}{1.1}, 0)$,
 an initial orientation $\theta_1 = \frac{\pi}{2}$ and a $\beta$ value varying between $\beta_1$, $\beta_2$ and $\beta_3$.\\
The right one has the same radius $a$, coordinates at $(\frac{a}{1.1}, 0)$,
 a $\beta$ value varying bewteen $\beta_1$, $\beta_2$ and $\beta_3$ and an initial orientation $\theta_2$ varying among
...
directional vector for $\beta \leq 0$, which do not correlate with the previous study.
