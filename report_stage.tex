...
This internship is a continuation and an improvement of the project done during the first year of Master.
The goal is to study the interactions and collective behaviors of squirmers.\\

\subsection{Eo parameter}
The first thing done during the internship was to study the effect of the $Eo$ parameter on the alignment of the squirmers.\\
...
and modify the \texttt{Code/run.py} file with the parameters one wants to test,
 then for each $Eo$ in \texttt{Code/run.py} and each $orient2$ and $betas$ in \texttt{sim\_Eo\_param} in \texttt{Code/simulation.py}, 
...
$$
with $\hat{e}_{ij} = \frac{(x_j - x_i)}{||x_j - x_i||}$, $e_{ij} = \hat{e}_i\cdot \hat{e}_{ij}$ and $Eo$ the parameter to test.\\ 
For squirmers having the same $\beta$ parameter we note the following relation:
$$
\Gamma_{ji->i} = \frac{1}{4}\Gamma_{ij->i}
$$
We want to study the effect of $E_0$ on the alignment of squirmers so $\Gamma_{i}^W$, which is the torque 
exerted by a wall on the squirmer, will be ignored.

\subsubsection*{Numerical Experiments}
The simulation are done with two squirmers of radius $a=0.05$, velocity $v_0=1$, $\beta$ varying from $\beta_1 = 0$,
 $\beta_2 = -7.5$ and $\beta_3 = 7.5$, orientation $\theta$ varying 
