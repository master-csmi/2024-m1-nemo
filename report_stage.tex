...
In our case the squirmers don't have inertia, so $\vec{a} = 0$, such as:
...
= -9 \mu \pi r \frac{1}{4}\frac{3}{2}v_0\left[ -p_i\hat{R}_{ij} -\beta (p_i\hat{R}_{ij})^2 + \frac{1}{2} \beta(p_i\hat{R}^{\perp}_{ij})^2\right](ln \epsilon + O(1))
\end{align*}
\begin{equation*}
\boxed{
...
\begin{itemize}
   ...
   takes \texttt{R} half of the length of the square, \texttt{history} the list of \texttt{loop\_time}, 
   ...
   It then plots the evolution of each squirmer's position over time, an arrow is plotted every two times the orientation 
   is modified and a circle is plotted every four time the orientation is modified,
